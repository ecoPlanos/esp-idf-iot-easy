...
#define FLOW_RESOLUTION_MV (float)CONFIG_YF_B3_SUPPLY_VOLTAGE/(MAX_FLOW - MIN_FLOW + 1.0)

static const char *TAG = "YF-B3";
...
esp_err_t yf_b3_init(analog_sen_t *yf_sen, uint8_t samples_filter, uint32_t period_ms, uint16_t sen_id, adc_unit_t water_unit, adc_channel_t water_channel, char *sen_name){
  CHECK_ARG(yf_sen);
  CHECK(analog_sen_init_desc(yf_sen, samples_filter, period_ms, sen_id, sen_name, 1, yf_b3_calc_water_flow));
